...
grep -Fq 'go1.26.6.darwin-amd64.tar.gz' scripts/bootstrap-go-toolchain.sh || fail "Intel toolchain URL is not pinned"
grep -Fq '08b65a63f244115121ced6c3b55ad38d801a7442acad5c949a17aad84ae6d684' scripts/bootstrap-go-toolchain.sh || fail "Intel toolchain digest is not pinned"
grep -Fq '67851334' scripts/bootstrap-go-toolchain.sh || fail "Intel toolchain size is not pinned"
grep -Fq 'go1.26.6.darwin-arm64.tar.gz' scripts/bootstrap-go-toolchain.sh || fail "arm64 toolchain URL is not pinned"
grep -Fq '2dc95ce4675829f2df0e86b28bcef3283635902062a5f0580ca659bf570f3204' scripts/bootstrap-go-toolchain.sh || fail "arm64 toolchain digest is not pinned"
grep -Fq '64772321' scripts/bootstrap-go-toolchain.sh || fail "arm64 toolchain size is not pinned"
...
for bootstrap_arch in arm64 x86_64; do
  ...
  MOCK_BOOTSTRAP_LOG="$tmp/bootstrap.log" MOCK_BOOTSTRAP_ARCHIVE="$bootstrap_archive" MOCK_UNAME_ARCH="$bootstrap_arch" \
    ...
    EXPECTED_ARCHIVE_URL="https://dl.google.com/go/go1.26.6.darwin-$archive_arch.tar.gz" \
...
#!/usr/bin/env bash
set -euo pipefail
if [[ "$1" == env && "$2" == GOVERSION ]]; then printf 'go1.26.6\n'; exit 0; fi

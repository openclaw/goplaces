...
grep -Fq 'govulncheck@v1.8.0' "$workflow" || fail "govulncheck version is not pinned"
...
grep -Fq 'go1.26.8.darwin-amd64.tar.gz' scripts/bootstrap-go-toolchain.sh || fail "Intel toolchain URL is not pinned"
grep -Fq '186be014105aa6542b767d2c6ed5cca10a0214bdff809ef1724022a8c7894150' scripts/bootstrap-go-toolchain.sh || fail "Intel toolchain digest is not pinned"
grep -Fq '67759394' scripts/bootstrap-go-toolchain.sh || fail "Intel toolchain size is not pinned"
grep -Fq 'go1.26.8.darwin-arm64.tar.gz' scripts/bootstrap-go-toolchain.sh || fail "arm64 toolchain URL is not pinned"
grep -Fq 'a012b25b571bd0138a03dcd25375ceba866fe5ca822f426d2c66a4de56fd3f4b' scripts/bootstrap-go-toolchain.sh || fail "arm64 toolchain digest is not pinned"
grep -Fq '64626620' scripts/bootstrap-go-toolchain.sh || fail "arm64 toolchain size is not pinned"
...
for bootstrap_arch in arm64 x86_64; do
  ...
  MOCK_BOOTSTRAP_LOG="$tmp/bootstrap.log" MOCK_BOOTSTRAP_ARCHIVE="$bootstrap_archive" MOCK_UNAME_ARCH="$bootstrap_arch" \
    ...
    EXPECTED_ARCHIVE_URL="https://dl.google.com/go/go1.26.8.darwin-$archive_arch.tar.gz" \
